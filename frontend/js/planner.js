...
let rooms = [], openings = [], selected = null, selectedKind = null;
let currentTool = 'select', drawing = false, start = null, cursor = null;
...
function undo() { if (!history.length) return; const state = JSON.parse(history.pop()); rooms = state.rooms; openings = state.openings; selected = null; selectedKind = null; draw(); updateProps(); }
function deleteSelected() {
    ...
    selected = null; selectedKind = null; draw(); updateProps();
}
...
canvas.addEventListener('pointerdown', event => {
    ...
    else if (currentTool === 'select') { selected = findOpening(point); selectedKind = selected ? 'opening' : null; if (!selected) { selected = findRoom(point); selectedKind = selected ? 'room' : null; } updateProps(); draw(); }
    else addOpening(currentTool, point);
    draw();
});
canvas.addEventListener('pointermove', event => { if (drawing) { event.preventDefault(); cursor = pnt(event); draw(); } });
...
function finishDrawing(event) {
    ...
    if (width >= 1 && height >= 1) { pushHistory(); selected = { id: crypto.randomUUID(), name: document.getElementById('roomName').value || 'Room', x, y, width, height, wall_thickness: +document.getElementById('wallThickness').value || .23, floor_height: +document.getElementById('roomHeight').value || 3 }; rooms.push(selected); selectedKind = 'room'; }
...
function updateProps() {
    ...
    if (selectedKind === 'opening' && selected) {
        ...
        title.textContent = 'Selected Room'; props.innerHTML = `<p><b>${selected.name}</b></p><p>${selected.width.toFixed(2)} × ${selected.height.toFixed(2)} m</p><p>Area ${(selected.width * selected.height).toFixed(2)} m2</p>`;
...
function applyPlan(plan) { rooms = plan.rooms || []; openings = plan.openings || []; selected = null; selectedKind = null; projectName.value = plan.project_name || 'AI Reconstructed Plan'; floors.value = plan.floors || 1; floorToFloor.value = plan.floor_to_floor || 3; confidence.innerHTML = `<b>AI confidence:</b> ${Math.round((plan.confidence || 0) * 100)}%<br>${(plan.notes || []).map(note => `<p>${note}</p>`).join('')}`; fitPlan(); updateProps(); }
async function analyzeLayout() { const response = await fetch('/api/analyze', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(getProject()) }); const data = await response.json(); if (!response.ok) return alert(JSON.stringify(data.detail)); confidence.innerHTML = `<b>Validated</b><p>Area ${data.total_area} m2</p><p>Building ${data.building_width} × ${data.building_depth} m</p>`; }
...
async function loadProject() { const id = prompt('Project ID:'); if (!id) return; const response = await fetch('/api/projects/' + id); const data = await response.json(); if (!response.ok) return alert(data.detail); applyPlan(data); }
